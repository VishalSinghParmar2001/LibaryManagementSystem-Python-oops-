...
if __name__ == "__main__":
    ...
    Indorelibrary = Library(
        ["The host", "Myth to Mithya", "IKIGAI", "Wish I Could Tell You", "Eleven Minutes", "A Brief History of Time"])
    student = Student()
    track = []

    print("\t\t\t\t\t\t\t♦♦♦♦♦♦♦ WELCOME TO THE INDORE LIBRARY ♦♦♦♦♦♦♦\n")
    print(
    ...
    while (True):
        ...
        try:
            ...
            if usr_response == 1:  # listing
                Indorelibrary.displayAvailableBooks()
            elif usr_response == 2:  # borrow
                Indorelibrary.borrowBook(
                    input("Enter your name: "), student.requestBook())
            elif usr_response == 3:  # return
                Indorelibrary.returnBook(student.returnBook())
            elif usr_response == 4:  # donate
                Indorelibrary.donateBook(student.donateBook())
